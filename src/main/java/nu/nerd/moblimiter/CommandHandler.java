...
public class CommandHandler implements CommandExecutor {
    ...
    private void reloadConfig(CommandSender sender) {
        ...
        if (!sender.hasPermission("moblimiter.reload")) {
            ...
            sender.sendMessage(
                    Component.text("You don't have permission to do this.").color(TextColor.fromHexString("#FF5555")));
            return;

        }

        plugin.getConfiguration().load();
        sender.sendMessage(Component.text("MobLimiter config reloaded!").color(TextColor.fromHexString("#FFAA00")));
    ...
    /**
     * When a player runs /moblimiter with no arguments, list available commands and
     * guide newbies to /moblimiter help
     *
     * @param sender
     */
    ...
    private void helpText(CommandSender sender, Command cmd, String[] args) {
        ...
        if (num == 1) {
            ...
            helpMessage = helpMessage
                    .append(LegacyComponentSerializer.legacyAmpersand().deserialize("&e1. Entity unload culling"))
                    .appendNewline()
            ...
            helpMessage = helpMessage
                    .append(LegacyComponentSerializer.legacyAmpersand().deserialize("&e2. Real-time spawn limiting"))
                    .appendNewline()
                    .append(LegacyComponentSerializer.legacyAmpersand()
        ...
        if (num == 2) {
            ...
            sender.sendMessage(
                    LegacyComponentSerializer.legacyAmpersand().deserialize("&e3. Age limiting").appendNewline()
                            .append(LegacyComponentSerializer.legacyAmpersand()
    ...
    private Component countMessager(HashMap<String, Integer> map, String header) {
        ...
        for (Map.Entry<String, Integer> entry : map.entrySet()) {
            ...
            countMessage = countMessage
                    .append(Component.text("&f" + entry.getKey().toLowerCase() + ": &7" + entry.getValue() + " "));
    ...
    private void limitsCommand(CommandSender sender, String[] args) {
        ...
        String values = String.format("Age: %d Max: %d Chunk: %d Cull: %d", d.getAge(), d.getMax(), d.getChunkMax(),
                d.getCull());
        ...
        for (ConfiguredMob l : limits.values()) {
            ...
            values = String.format("Age: %d Max: %d Chunk: %d Cull: %d", l.getAge(), l.getMax(), l.getChunkMax(),
                    l.getCull());
        ...
        if (lines.size() > 0) {
            ...
            sender.sendMessage(LegacyComponentSerializer.legacyAmpersand()
                    .deserialize(String.format("%s[Page %d/%d]", "&7", page, pages)));

        } else {

            sender.sendMessage(Component
                    .text("There are no limits configured. All mob types will fall back" + " to the default block.")
    ...
    private void checkCommand(CommandSender sender) {
        ...
        sender.sendMessage(LegacyComponentSerializer.legacyAmpersand().deserialize(String
                .format("&7 (%d/%d nearby, %d/%d in chunk)", nearby, limits.getMax(), inChunk, limits.getChunkMax())));
        ...
        String chunkCullStr = (limits.getCull() > -1 && isCullable) ? String.format("%d", limits.getCull())
                : "Unlimited";

        sender.sendMessage(
                LegacyComponentSerializer.legacyAmpersand().deserialize(String.format("Age: %s%s", "&7", ageStr)));
        sender.sendMessage(LegacyComponentSerializer.legacyAmpersand()
