...
public class ClickEvents implements Listener {
    ...
    /**
     * Formerly part of LimitSpawnEggs Stops players from changing the mob of a
     * spawner with spawn eggs
     */
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getClickedBlock().getType() == Material.SPAWNER
                && event.getItem() != null && spawnEggs.contains(event.getItem().getType()))
        {
    ...
    /**
     * Formerly part of KeepBabyMobs Handles the locking of baby mobs. Originally
     * handled by KeepBabyMobs.
     */
    @EventHandler
    ...
    public void logLock(Player player, Entity entity) {
        ...
        MobLimiter.instance.getComponentLogger()
                .info(Component.text(String.format("%s age locked %s named %s at %s", player.getName(),
                        entity.getType(), entity.customName(), MobLimiter.locationToString(entity.getLocation()))));
