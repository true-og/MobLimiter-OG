...
public class EntityEvents implements Listener {
    ...
    /**
     * Formerly part of KeepBabyMobs. Logs the death of an entity.
     */
    ...
    public void logDeath(Player player, Entity entity, String extrainfo) {
        ...
        MobLimiter.instance.getComponentLogger()
                .info(Component.text(String.format("%s killed %s named %s at %s %s", player.getName(), entity.getType(),
                        PlainTextComponentSerializer.plainText().serialize(entity.customName()),
                        MobLimiter.locationToString(entity.getLocation()), extrainfo)));
    ...
    public void onCreatureSpawnEvent(final CreatureSpawnEvent event) {
        ...
        if ((reason == CreatureSpawnEvent.SpawnReason.BREEDING || reason == CreatureSpawnEvent.SpawnReason.EGG
    ...
    /**
     * Controls the rate farm animals grow up and the length of their breeding
     * cooldown. growthTicks: How many ticks until the animal becomes an adult.
     * breedingTicks: How many ticks until the animal can breed again. A value of
     * zero makes the condition instant, and -1 disables tampering with vanilla
     * behavior. This functionality was called "agecapbaby" and "agecapbaby"
     * respectively in MobLimiter 1.x.
     * 
