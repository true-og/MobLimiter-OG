...
/**
 * Sweep loaded chunks every 30 seconds and remove applicable mobs over their
 * type's age limit
 */
public class AgeLimiter extends BukkitRunnable implements Listener {
    ...
    private void sweepChunk(Chunk chunk) {
        ...
        for (Entity entity : chunk.getEntities()) {
            ...
            if (entity.isDead() || !EntityHelper.isLimitableMob(entity) || entity instanceof Villager)
                continue;
    ...
    /**
     * Don't let the sweep kill entities currently targeting a player. Also, keep
     * resetting the relative age, since EntityTargetEvent only fires when the
     * target changes.
     * 
    ...
    private boolean isTargetingPlayer(Entity entity) {
        ...
        if (!plugin.getConfiguration().relativeAgeEnabled())
            return false;
    ...
    /**
     * Returns the adjusted age of a mob. If relative age is on, and the mob
     * recently tageted a player, the delta ticks will be returned. Otherwise, the
     * regular getTicksLived() will be used.
     * 
     * @param entity the entity to check
