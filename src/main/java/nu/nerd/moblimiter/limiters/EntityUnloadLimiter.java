...
/**
 * Cull applicable mobs on chunk unload. This is based on behavior from the
 * original MobLimiter 1.x.
 */
public class EntityUnloadLimiter implements Listener {
    ...
    /**
     * Remove excess mobs in a chunk, if chunk unload culling is enabled for the mob
     * type. This is the default behavior from MobLimiter 1.x.
     * 
    ...
    private void removeMobs(List<Entity> entities) {
        ...
        for (Entity entity : entities) {
            ...
            if (cap > -1 && mobCount > cap) {
                ...
                EntityHelper.notifyDespawn(entity, "chunk unload cull");
                entity.remove();
                if (plugin.getConfiguration().debug()) {
